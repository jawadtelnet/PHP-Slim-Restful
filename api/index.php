...
function signup() {
    ...
    try {
        ...
        //echo $username_check.'---'.$username.'<br/>';
		//echo $email_check.'<--->'.$email.'<br/>';
		//echo $password_check.'<--->'.$password.'<br/>';
...
function feedUpdate(){
    ...
    try {
        ...
        if($systemToken == $token){
            ...
            $feedData = '';
            $db = getDB();
            $sql = "INSERT INTO feed ( feed, created, user_id_fk) VALUES (:feed,now(),:user_id)";
            $stmt = $db->prepare($sql);
